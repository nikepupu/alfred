...
class ContrastiveSWM(nn.Module):
    ...
    def __init__(self, embedding_dim, input_dims, hidden_dim, action_dim,
        ...
        self.obj_encoder = EncoderMLP(
            ...
            num_objects=num_objects,
            language_dim_high=ROBERTA_DIM ,
            language_dim_low=ROBERTA_DIM 
            )
    ...
    def contrastive_loss(self, obs, action, next_obs, low_desc, task_desc):
       
        language_encoding1 = self.lstm_low(low_desc )[0][:,-1,:]
        language_encoding2= self.lstm_task(task_desc)[0][:,-1,:]
        
        objs = self.obj_extractor(obs)
        next_objs = self.obj_extractor(next_obs)
      
        state = self.obj_encoder(objs, language_encoding1, language_encoding2)
        # next_state = self.obj_encoder(next_objs)
        
        next_state = self.obj_encoder(next_objs, language_encoding1, language_encoding2)
...
class EncoderMLP(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, hidden_dim, num_objects, language_dim_low, language_dim_high,
                 act_fn='relu'):
        super(EncoderMLP, self).__init__()

        self.num_objects = num_objects
        self.input_dim = input_dim
        self.language_dim_low = language_dim_low
        self.language_dim_high = language_dim_high
        
        self.fc1 = nn.Linear(self.input_dim+self.language_dim_low+self.language_dim_high, hidden_dim)
    ...
    def forward(self, ins, task_desc, low_desc):
        
        task_desc = task_desc.unsqueeze(1)
        low_desc = low_desc.unsqueeze(1)
        h_flat = ins.view(-1, self.num_objects, self.input_dim)
        # print(task_desc.shape)
        # print(low_desc.shape)
        task_desc= task_desc.repeat(1, self.num_objects, 1)
        low_desc = low_desc.repeat(1, self.num_objects, 1)
        # print(task_desc.shape)
        # print(low_desc.shape)
        # print(h_flat.shape)
        h_flat = torch.cat((h_flat, task_desc, low_desc), 2)
...
if __name__ == "__main__":
    ...
    loss = world_model.contrastive_loss(ob, action, next_obs, low_descs, task_desc )